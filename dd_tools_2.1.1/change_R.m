%CHANGE_R Auxiliary function for incSVDD
%
...
function R = change_R(R,c,beta,gammac,dw, smalleps)
...
% 		R = [R zeros(n,1); zeros(1,n+1)] + ...       %%original
% 			([beta;1]/gammac)*[beta' 1];
        
         R = [R zeros(n,1); zeros(1,n+1)] + ...  % adiciona o peso
			dw*(([beta;1]/gammac)*[beta' 1]);
...
% 			([beta;1]/(sign(gammac)*smalleps))*[beta' 1];
         ...
         R = [R zeros(n,1); zeros(1,n+1)] + ...
			dw*([beta;1]/(sign(gammac)*smalleps))*[beta' 1]; % adiciona o peso
	end
