...
%   NEWX          object vector
%   NEWY          object label
%   ALREADYADDED  Flag if object is in support vector structure
%
% OUTPUT
%   W             support vector structure
%
...
%
% SEE ALSO
% inc_setup, incsvc, incsvdd, inc_add, inc_store

% Copyright: D.M.J. Tax, D.M.J.Tax@prtools.org
% Faculty EWI, Delft University of Technology
% P.O. Box 5031, 2600 GA Delft, The Netherlands

function W = inc_add(W,newx,newy,newdw,alreadyadded)
...
if alreadyadded
    ...
    switch W.type
        case 'svdd'
            Kc = 2*(W.y(c)*W.y(newsetD)').*K;
            W.grad(c,1) = W.y(c)*W.b - W.y(c)*Kc(c)/2; % calculo Multiplicador de Langrange
        case 'svc'
            Kc = (W.y(c)*W.y(newsetD)').*K;
            W.grad(c,1) = W.y(c)*W.b - 1;
            
    end
    if ~isempty(W.setS)  % se ja nao existe suporte vector
        W.grad(c,1) = W.grad(c,1) + Kc(W.setS)*W.alf(W.setS);% calculo Multiplicador de Langrange
    end
    if ~isempty(W.setE)
        W.grad(c,1) = W.grad(c,1) + Kc(W.setE)*W.alf(W.setE); % calculo Multiplicador de Langrange
    end
end

if W.grad(c)>0 % Multiplicadores de Lagrange (indica a direcao da amostra)
    ...
    while ~done
        ...
        W.grad = W.grad + (mindelta-duptonow)*gamma; % calcula a direcao e o sentido
        ...
        switch situation
            ...
            case 2 % obj c goes to setS: SV
                ...
                if isempty(W.setS) % compute it directly (to avoid inf)
                    ...
                    W.R = change_R(W.R,+c,beta,gamma(c), newdw);
                end
                W.setS = [W.setS; c];
                done = 1;
            case 3 % a support object hits upper bound
                ...
                W.R = change_R(W.R, -nrS_up,beta,gamma(j), newdw);
            case 4 % a support object hits lower bound
                ...
                W.setS(nrS_low) = [];
                W.R = change_R(W.R,-nrS_low,beta,gamma(j), newdw);
            case 5 % an error becomes a support object   
                ...
                if length(betaj)==1
                    ...
                    W.R = change_R(W.R, +j, betaj, gammaj, newdw);
                end
